...
function async.map_concurrent(items, worker_fn, max_workers, progress_fn)
    ...
    while completed < #items do
        ...
        while #active < max_workers and item_idx <= #items do
            ...
            -- Create lane worker
            local worker = lanes.gen("*", worker_fn)(item)
        ...
        local i = 1
        while i <= #active do
            local lane_obj = active[i].lane
            local status = lane_obj.status
            
            if status == "done" then
                -- Get result
                local ok, result = pcall(function() return lane_obj[1] end)
                if ok then
